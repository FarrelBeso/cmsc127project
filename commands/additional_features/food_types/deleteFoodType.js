...
/**
 * Delete type from food.
 */
export async function deleteFoodType() {
  ...
  try {
    ...
    // delete to db
    spinner = ora("Delete type...").start();
    await conn.query("DELETE FROM food_item_type WHERE food_id=? AND type=?", [
      foodIdPrompt.id,
      typePrompt.type,
    ]);
    spinner.stop();
    ...
    // confirm operation
    console.log(chalk.greenBright("Type deleted successfully!"));
